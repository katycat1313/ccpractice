...
/**
 * Enhanced Gemini Voice Agent Hook
 * Manages bidirectional audio conversation with Gemini API
 * @param {Object} options - Configuration options
 * @param {string} options.apiKey - Gemini API key (required)
 * @param {string} options.voiceName - Voice name (default: 'Puck')
 */
export const useGeminiVoiceAgent = ({ apiKey, voiceName = 'Puck' } = {}) => {
    ...
    const startConversation = useCallback(async (systemPrompt = 'You are a helpful assistant.') => {
        if (!apiKey) {
        ...
        try {
            console.log('[Gemini] Starting conversation');
            setError(null);
            setMessages([]);
            ...
            const handleGeminiMessage = (message, stream, ws, audioContext) => {
                if (message.setupComplete) {
                    ...
                    for (const part of parts) {
                        ...
                        if (part.text) {
                            setMessages(prev => [...prev, { id: Date.now(), speaker: 'AI', text: part.text, timestamp: Date.now() }]);
                        }
                    }
                }
            };
